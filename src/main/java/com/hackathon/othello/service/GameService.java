...
public class GameService {
    ...
    public boolean estCoupValide(int ligne, int colonne, int joueur) {
        ...
        // Vérifie dans toutes les directions s'il y a au moins un pion adverse suivi d'un pion du joueur
        for (int[] dir : DIRECTIONS) {
            ...
            // Si on a trouvé au moins un pion adverse et qu'on arrive sur un pion du joueur, le coup est valide
    ...
    public List<int[]> getCoupsValides(int joueur) {
        // Chaque coup valide est représenté par un tableau de deux éléments : [ligne, colonne]
    ...
    public boolean jouerCoup(int ligne, int colonne, int joueur) {
        ...
        if (!estCoupValide(ligne, colonne, joueur)) return false;
    ...
    public int getVainqueur() {
        ...
        if (noir > blanc) return 1;
        if (blanc > noir) return 2;
        return 0;
    }
